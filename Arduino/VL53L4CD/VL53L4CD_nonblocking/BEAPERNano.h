...
const uint8_t TFT_DC = 9;    // TFT DC (Data/Command) pin
const uint8_t TFT_CS = 10;   // TFT CS (Chip Select) pin
const uint8_t TFT_COPI = 11; // TFT SDA data pin (SPI MOSI/COPI)
const uint8_t TFT_BL = 12;   // TFT BLK (backlight control) pin (unused Nano CIPO pin)
const uint8_t TFT_SCK = 13;  // TFT SCL clock pin (shared with Nano LED_BUILTIN)
const int     TFT_RST = -1;  // TFT RES reset pin tied to BEAPER Nano hardware reset


/* =====================================
 * Utility Functions (added in Activity I03)
 * ====================================*/

// map_range() maps a value from one numeric range to another.
// Returns a float. Similar to Arduino's map() but works with
// floating-point values and non-integer ranges.
// If in_min == in_max, returns out_min to avoid division by zero.
inline float map_range(float value, float in_min, float in_max,
...
// led_bar() displays a proportional 4-LED bar graph for level
// within [min_val, max_val]. Uses map_range() and constrain().
// 0 LEDs = at or below min_val; 4 LEDs = at or above max_val.
inline void led_bar(int level, int min_val = 0, int max_val = 1023)
{
    float scaled = constrain(map_range(level, min_val, max_val,
                                       0.0f, 4.0f), 0.0f, 4.0f);
    digitalWrite(LED2, scaled >= 1.0f);
    digitalWrite(LED3, scaled >= 2.0f);
    digitalWrite(LED4, scaled >= 3.0f);
    digitalWrite(LED5, scaled >= 4.0f);
}
