...
const int LIGHT_HIGH  = 45000;        // Light threshold: LED turns on above this
const int LIGHT_LOW   = 35000;        // Light threshold: LED turns off below this
...
float read_temp_c()
{
    ...
    float voltage = (float)analogRead(U4) * 3.3 / 65535.0;
...
void setup()
{
    ...
    // Configure the ADC for 16-bit readings (0-65535). The ESP32-S3's
    // ADC hardware is natively 12-bit; this pads the reading rather than
    // adding real precision - see GE1 for more on what this means.
...
void loop()
{
    ...
    int frequency = (int)map_range(rv2, 0, 65535, TONE_MIN, TONE_MAX);
    tone(LS1, frequency);
...
integer and float numeric types in detail - the Numeric Types document
...
This program calls 'analogReadResolution(16)' in setup(), so the
Arduino Nano ESP32's ADC returns values from 0 to 65535 - a 16-bit
range. This means readings are scaled to divide the full input
voltage range (0 to 3.3V) into 65536 steps, giving an apparent
resolution of about 0.05mV per step. The physical ADC resolution
is 12 bits, or only about 0.8mV per step. Using 16-bit results
enables analog algorithms to easily scale when future devices
have higher ADC resolution.

When RV1 is turned fully anti-clockwise, 'analogRead(RV1)' returns 0.
Fully clockwise returns 65535. Observe the Serial Monitor while slowly
rotating RV1 from one extreme to the other. Is the change perfectly
smooth? Do you notice any irregularities?

Why do you think the ADC use a range of 0-65535 specifically, rather
than 0-100 or 0-1000? Think about how 65536 relates to powers of 2 and
the binary number system from Activity 7. The Numeric Types document
explains why integer sizes are always powers of 2.
...
In this program 'map_range()' is used to create an audio frequency.
Trace through this formula with value=32767 (approximately halfway),
in_min=0, in_max=65535, out_min=200, out_max=4000. What frequency
...
Trace through the function with max_level=65535 and these values of
level: 0, 8192, 24576, 40960, 57344, 65535. Which LEDs are lit for
each value?
...
results are also integers. You will explore this further in GE 5.
...
uses '(float)analogRead(U4)' before dividing: without the cast, the
division would be integer arithmetic and the temperature conversion
...
Celsius using the MCP9700A temperature sensor's characteristics. The
sensor outputs 0.5V at 0°C, and the voltage increases by 10mV (0.01V)
for every degree Celsius above zero.
...
Why is the raw value multiplied by 3.3 and divided by 65535? Note
that 65535.0 (with a decimal point) ensures float division even
without an explicit cast - what would happen if it were written as
65535 instead?
...
At what raw ADC value would the sensor read exactly 0°C? Calculate it
and verify by adding a temporary Serial.print inside 'read_temp_c()'.

The board has been running for a while when you take a reading.
Would you expect U4 to read exactly room temperature, or slightly
warmer? What does this mean for how accurately U4 measures the
...
thresholds - a higher one to turn something on, and a lower one to turn
it off. Using two separate thresholds prevents flickering when a reading
hovers near a single boundary. GE 8 covers this in detail; first, take
...
Run the program and slowly cover Q4 completely with your finger, then
...
A single threshold would turn the status LED on when light level
exceeds it and off when light level falls below it, but when the
light level hovers near the boundary, the LED flickers rapidly as
the actual reading crosses back and forth over the threshold value.

Hysteresis solves this using two thresholds: the LED turns on only
when light level rises above LIGHT_HIGH, and turns off only when it
falls below LIGHT_LOW. The gap between them creates a band of values
where the LED holds its current state regardless of small fluctuations.
...
LIGHT_LOW to approximately 25%. Slowly cover and uncover Q4 while
...
Call 'show_bar(light, 65535, true)' for the light display and verify
...
that avoids constant switching due to sensor noise? The BEAPER Nano
uses U4 as its only temperature sensor - how might the board's own
heat output affect the readings over time, and what does this mean
for measuring slow changes in ambient temperature?
...
for RV1, light level, and temperature since the program started.
Print the current value, minimum, and maximum for each sensor on
every iteration. When SW5 is pressed, reset all minimums and maximums
...
Call this function once in setup() for RV1 and the light sensor
analog pins, storing the results as baseline values. In loop(),
display the difference between the current reading and the baseline
...
Build a simple environmental monitor using all available analog
inputs. Display temperature in degrees C and light as a percentage of
the calibrated maximum. Use RV1 to set a temperature alarm threshold
and RV2 to set a light alarm threshold. When either sensor exceeds
its threshold, sound a distinctive alarm tone and flash the
corresponding LED.
