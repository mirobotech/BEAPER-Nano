...
The pushbuttons in the BEAPER Nano circuit are connected in what is
...
'if (digitalRead(SW2)) = LOW' would cause an error, while
'if (digitalRead(SW2)) == LOW' checks whether SW2 is reading LOW.

What value do you expect digitalRead(SW2) to return when SW2 is not
pressed? What about when SW2 is pressed?
...
Looking at SW2's if-else condition, explain the program flow when SW2
is not pressed, and when SW2 is pressed. Which digitalWrite() call
executes in each case?

Notice how comments like '// Momentary button SW2' and '// Start a
