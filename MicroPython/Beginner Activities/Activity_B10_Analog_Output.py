...
while True:
  ...
  # Once triggered, LED3 fade continues without the button being held
...
# physical quantities from sensors, and converting the raw ADC values
...
# connected to the output, and the PWM frequency is high enough that
# the individual pulses cannot be seen, the LED will be perceived as
# being steadily lit at about half brightness.
#
...
# The Numeric Types document explains why hardware ranges are
# expressed as powers of 2.
#
# --------------------------------------------------------------------------------
# GE 2 - Stepping outside the board module's abstraction
# --------------------------------------------------------------------------------
#
# The PWM objects for the LED pins are created using pin number
...
# LED2_PWM = PWM(Pin(beaper.LED2_PIN), freq=PWM_FREQ, duty_u16=0)
#
# Open BEAPER_Nano.py and find where both 'LED2_PIN' and 'LED2'
...
# --------------------------------------------------------------------------------
#
# The PWM frequency (set to 1000 Hz by PWM_FREQ) determines how
...
#
# Note that changing PWM_FREQ requires restarting the program, since
# PWM objects are created with the frequency at startup. You can update
# frequency on a running object using 'LED2_PWM.freq(new_freq)' - try
# this in the console while the program is running.
#
# At what frequency does the flicker become invisible to you? Why do
# motor controllers typically use PWM frequencies of 10 kHz or higher
# even though motors have no visual response? Hint: what might you
# hear if a motor were driven at 500 Hz?
#
# --------------------------------------------------------------------------------
...
# of 0 or above the maximum PWM value of 65535? What do you notice
...
# --------------------------------------------------------------------------------
#
# This program runs in a continuous 'while True:' loop, checking
...
#
# Refer to: GE 1
# Open: B10_RGB_Controller_Project.py
