...
while True:
  ...
  # --- State: Follow ---
    ...
    if not any_on_line():
      ...
      # If the right sensor (Q3) was last on, the line is to the
      # right - turn right. Otherwise turn left.
...
# --------------------------------------------------------------------------------
#
# Before any state logic, calibrate your line sensors. Add a
# temporary calibration loop after the startup sensor check:
#
# Example code:
#
# while True:
...
# over the dark tape. Set LINE_THRESHOLD to a value comfortably
# midway between the two. A good threshold leaves a clear gap on both
# sides so that minor lighting variation does not trigger false
# readings.
#
...
# follows by keeping floor on both sides (both sensors off the
# line). This handles wider lines, T-junctions, and + intersections
...
#
# Invert the steering logic: if both sensors are HIGH (on the
# floor), drive straight. Correct when one sensor goes LOW (over the
# line).
#
# --------------------------------------------------------------------------------
...
#
# This gives a true three-sensor arrangement where Q3 tracks the
# line centre and Q2/Q1 detect drift to either side. Priority-based
# steering becomes possible: outer sensors (Q2, Q1) take priority
# over the centre sensor (Q3), and the robot can detect
# intersections when all three trigger simultaneously.
#
...
# --------------------------------------------------------------------------------
#
# Rather than stopping a motor for corrections, reduce its duty
# cycle proportionally. This produces smoother following and reduces
# oscillation on straight sections. Investigate how to set PWM duty
# cycle on motor pins using beaper.M1A.duty_u16() and compare the
# result to the binary approach - see Activity 10's Motor Controller
# project for the PWM speed-control pattern this builds on.
#
# --------------------------------------------------------------------------------
# EA 2 - Lap counter
# --------------------------------------------------------------------------------
...
# for more than MARKER_TIME ms without having been at a recovery
# event recently. Print the lap count and lap time each lap.
#
# --------------------------------------------------------------------------------
# EA 3 - Course map
# --------------------------------------------------------------------------------
