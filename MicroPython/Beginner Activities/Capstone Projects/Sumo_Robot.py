# ================================================================================
# Capstone Project: Sumo Robot [Sumo_Robot.py]
# Version: 1.0
...
#   JP4 = Robot   (connects VDIV battery monitor to ADC3)
#
...
#   IDLE      - Robot stationary. Press SW2 to begin the match. Flash
#               the Arduino LED to show the robot is ready. SW5 does
...
#               slow rotation to sweep the SONAR across the ring. If
#               SONAR detects an opponent, transition to PUSH. If a
#               floor sensor trips, transition to EDGE.
#
#   PUSH      - Opponent detected within DETECT_RANGE cm. Robot drives
#               forward at full speed to push the opponent out. SONAR
#               is checked continuously: if opponent disappears (pushed
#               out or dodged), return to SEARCH. If a floor sensor
#               trips, transition to EDGE immediately.
#
...
# --- Timing Constants -------------------
START_DELAY    = const(5000)         # Mandatory pre-match countdown (ms) - real sumo rules
...
def read_sonar(current_time):
  ...
  # Returns the distance in cm, or the previous reading if called too soon.
  global sonar_distance, last_sonar_time
  if time.ticks_diff(current_time, last_sonar_time) >= SONAR_INTERVAL:
    result = beaper.sonar_range(DETECT_RANGE + 20)
...
print("  SONAR range:", beaper.sonar_range(), "cm")
print()
...
while True:
  ...
  # --- State: Idle ---
    ...
    # TODO: implement a flashing pattern on the Arduino Nano ESP32
    #       on-board LED to show the robot is powered and waiting.
    #       Use the Activity 11 repeating-timer pattern.

    # Start match when SW2 pressed
    if beaper.SW2.value() == 0:
      beaper.nano_led_on()            # On-board LED stays on while match is active
      last_beep_time = current_time
      enter_state(STATE_COUNTDOWN, current_time, "SW2 pressed")

  # --- State: Countdown ---
  elif state == STATE_COUNTDOWN:
    # Pre-match mandatory delay. Robot is stationary. Beep once per
    # second to count down, then release into SEARCH when START_DELAY
    # elapses.
  ...
  # --- State: Search ---
    ...
    if check_edge():
      ...
      if opponent_detected():
        enter_state(STATE_PUSH, current_time, "opponent at " + str(sonar_distance) + "cm")
      else:
  ...
  # --- State: Edge ---
    ...
    #   "left"  - back up, then turn right
    #   "right" - back up, then turn left
    #   "both"  - straight back, then turn (arbitrary direction)
...
#         "SONAR:", beaper.sonar_range(), "cm")
#   time.sleep_ms(100)
#
# Place the robot on the ring surface and note the Q1 and Q3
# readings. Move the robot to the boundary tape and note the change.
# Set EDGE_THRESHOLD to a value midway between the two readings. Test
# with the same lighting conditions you will use during the match.
#
...
# --------------------------------------------------------------------------------
#
# Verify motor directions before connecting to drive wheels. Call
# each motor function individually and confirm that "forward" moves
# the robot in the intended direction:
#
# Example code:
#
...
# correct edge_direction is printed.
#
# --------------------------------------------------------------------------------
...
# quick succession while rotating slightly left and right, and
# compare which direction gives the shorter reading. This is a
# simplified radar sweep.
#
# --------------------------------------------------------------------------------
# EA 2 - Floor sensor ring check in PUSH
# --------------------------------------------------------------------------------
#
# If both floor sensors read the ring surface consistently, you are
# in a safe central position. If one sensor reads lower (approaching
# an edge) during a push, steer away from that side. This prevents
# chasing the opponent all the way to your own edge.
#
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
#
# VDIV_level() reads the battery voltage through a voltage divider
# (set JP4=Robot). Low battery causes erratic motor behaviour that
# can look like software bugs. Print the VDIV reading at startup and
# periodically during the match. Add a low-battery warning (beep
# and/or slow flashing LED) if the reading drops below a threshold
# you determine experimentally.
#
# --------------------------------------------------------------------------------
# EA 5 - Match timer
# --------------------------------------------------------------------------------
...
# and transition from any active state to STATE_STOPPED when the
# time since COUNTDOWN ended exceeds MATCH_DURATION. Print the match
