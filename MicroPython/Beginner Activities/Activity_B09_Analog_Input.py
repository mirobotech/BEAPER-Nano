...
beaper.nano_led_on()  # Status LED on
...
# --------------------------------------------------------------------------------
#
# The Arduino Nano ESP32's 12-bit ADC uses the read_u16() method to
...
# The physical ADC resolution is only 12 bits, or about 0.8mV per step.
# Using 16-bit results enables analog algorithms to easily scale
# when future devices have higher ADC resolution.
#
# The board module's analog helper functions return these raw
# ADC values directly. When RV1 is turned fully anti-clockwise,
# 'RV1_level()' returns 0. Fully clockwise returns 65535. Observe
# the console while slowly rotating RV1 from one extreme to the
# other. Is the change perfectly smooth? Do you notice any
# irregularities?
#
...
# Numeric Types document explains why integer sizes are always
# powers of 2.
#
# --------------------------------------------------------------------------------
...
# calculations as integers, which is appropriate since
# absolute precision isn't needed for the comparison and ADC
...
# --------------------------------------------------------------------------------
#
# The 'read_temp_c()' function converts the raw ADC reading to
# degrees Celsius using the MCP9700A temperature sensor's
# characteristics. The sensor outputs 0.5V at 0°C, and the voltage
# increases by 10mV (0.01V) for every degree Celsius above zero.
#
...
#
# U4 sits on the same circuit board as the ESP32 and the voltage
# regulator, both of which generate heat while the board is
# running. If the board has been powered on for a while before you
# take a reading, would you expect U4 to read exactly room
# temperature, or slightly warmer? What would you need to do to
# get an accurate ambient temperature reading with this sensor?
#
# --------------------------------------------------------------------------------
...
# a higher one to turn something on, and a lower one to turn it off.
# Using two separate thresholds prevents flickering when a reading
# hovers near a single boundary. GE8 covers this in detail; first,
# take some measurements to calibrate the thresholds to your
# environment.
#
...
# level exceeds it and off when light level falls below it, but
# when the light level hovers near the boundary, the LED flickers
# rapidly as the actual reading crosses back and forth over the
# threshold value.
#
# Hysteresis solves this using two thresholds: the LED turns on
# only when light level rises above LIGHT_HIGH, and turns off
# only when it falls below LIGHT_LOW. The gap between them creates
# a band of values where the LED holds its current state regardless
# of small fluctuations.
#
# The 'light_on' variable is declared before the main loop, not
# inside it. Why is this essential to the hysteresis behaviour? What
# would happen if 'light_on = False' were declared inside the loop?
#
# Using the minimum and maximum readings from GE7, set LIGHT_HIGH to
...
# watching the status LED. Does the LED respond without flickering
# near the transition?
#
...
# --------------------------------------------------------------------------------
#
# Modify 'show_bar()' so that it accepts an optional third argument
# 'invert' that defaults to False. When invert is True, the bar fills
# from right to left - LED5 lights first at low values, LED2 last at
# high values. Use this to display the light level inverted, so that
# a darker environment produces a fuller bar.
#
...
#
# Call 'show_bar(light, 65535, invert=True)' for the light display
# and verify it responds correctly. What does this tell you about how
# default parameters make functions more flexible without requiring
...
# --------------------------------------------------------------------------------
#
# Build a simple environmental monitor using all four analog
