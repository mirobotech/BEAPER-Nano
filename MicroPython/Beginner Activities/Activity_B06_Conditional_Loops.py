...
#
# A while loop's condition can be any expression that evaluates to
# True or False, and the loop will keep running only as long as that
...
# condition is an important part of program design, and this activity
# explores different options and when each one is most useful.
#
# --------------------------------------------------------------------------------
...
# SW5 is pressed, SW5_pressed becomes True, 'not SW5_pressed' becomes
...
# set to a starting value before the loop begins, it is checked in
# the loop's condition on every iteration, and it is changed inside
# the loop body. This combination - initialize, check, update - is
# the standard pattern for a loop control variable.
#
...
#
# --------------------------------------------------------------------------------
# GE 3 - The wait loop's control variable
# --------------------------------------------------------------------------------
#
# The first while loop doesn't count, but still needs to initialize,
...
# --------------------------------------------------------------------------------
#
# Notice that the program has code between and after the two while
# loops. The 'beaper.nano_led_on()' line runs after the first loop
# exits but before the second loop begins, and the countdown-complete
# signal runs only after both loops have finished.
#
# Most programs you have written so far have had all their code
# inside a 'while True:' loop, which means nothing placed after that
# loop ever runs. This program demonstrates that loops can exit, and
# that code placed after a loop runs exactly once when the loop
# finishes.
#
...
# them - inside an outer 'while True:' loop, just like all of your
# previous programs. After the countdown finishes and the done signal
# plays, the program should reset and wait for SW5 to be pressed again.
...
# --------------------------------------------------------------------------------
#
# The 'while not SW5_pressed:' loop in this program is a common and
...
# program needs to pause at a specific point until a condition is
# met.
#
# You have also seen 'not' used in if conditions in Activity 5. In
# both cases, 'not' inverts the truth value of whatever follows it.
...
# active-high, while the first two options hide that detail behind a
# meaningful name. Which do you find most readable, and why? What
# does your answer tell you about the value of meaningful variable
# names?
#
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
#
# The print() statements in this program reveal what is happening
# inside the loops in the console. Run the program and observe the
# console output as the countdown progresses.
#
# Add a print statement inside the 'while not SW5_pressed:' loop to
# show that the waiting loop is running. What do you observe about
# how quickly the messages appear compared to the countdown loop's
# messages? What accounts for the difference in speed?
#
# --------------------------------------------------------------------------------
# GE 7 - Infinite loops
# --------------------------------------------------------------------------------
...
# forever - exactly like 'while True:'. This is sometimes called an
# infinite loop, and is usually the result of a bug rather than
# intentional design.
#
# The countdown loop exits because 'count -= 1' eventually makes
# 'count' reach zero. But what if the condition were 'while count !=
# 0:' and count started at an odd number and decreased by 2 each
# iteration? Would the loop ever exit? Try it:
#
# Example code:
#
# count = 5
...
# Press the stop button to end the program, then change the condition
# to 'while count > 0:' and verify that it now exits correctly.
...
# indicate the button was pressed and released. Use print statements
# to show each phase - waiting for a press, waiting for release, and
# done.
#
# Edge detection from Activity 5 catches the moment a button
# transitions from not pressed to pressed, but it does so by
# comparing the current and previous loop states on every cycle. This
# press-and-release approach is different - it explicitly waits in a
# dedicated loop for each transition to complete before moving on.
...
# of times equal to the number of times SW3 was pressed before SW5
# was pressed. The program should:
#
...
#    time.sleep_ms(10) delay inside the loop so each iteration takes
#    a known 10ms - this makes the iteration count a reliable measure
#    of elapsed time
...
#
# Hint: MicroPython's built-in random number generator can be used to
# get a random integer in a range like this:
#
# Example code:
#
# import random
# delay_ms = random.randint(2000, 5000)  # Random value between 2000 and 5000
...
# B06_Combination_Lock_Project.py is a separate, open-ended project
# file that asks you to build a button-sequence lock, using a step
# counter and conditional loops from this activity's examples.
# The project is provided as a skeleton with TODOs, making it a
# good next step to build confidence and design projects yourself.