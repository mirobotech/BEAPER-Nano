...
1.  Let's analyze this program! Program analysis is an important skill
    ...
    Open this program in the Arduino Lab for MicroPython IDE (or a
    similar MicroPython IDE) and connect your BEAPER Nano to your
    computer. Use the Connect button to connect the Arduino Nano ESP32
    to the IDE, and then press the Run button in the IDE to start
    running the program. With the program running, press and release
    pushbutton SW2 on your BEAPER Nano circuit while observing its
    LEDs.
...
3.  The LED circuits in BEAPER Nano use 'active-high' logic, meaning
    that each LED is turned on when its output pin is set to 1, or
    connected to the power supply potential by its internal output
    transistor. In contrast, the individual elements of the RGB (red,
    green, blue) LED built into the Arduino Nano ESP32 microcontroller
    are controlled using 'active-low' logic. This means that current
    flows through each LED element when its associated output is made
    0, or electrically connected to the circuit's ground potential.

    The configuration statements near the top of the program both
    define and initialize the Arduino Nano ESP32's built-in RGB LED.
    The Pin constructors' initialization values can be modified to
    display a different colour on startup, or to keep the LED off
    entirely, by simply changing their value. Let's do an experiment
    to determine all of the possible colours the RGB LED can create.
    To help you figure it out, create a chart with columns for red,
    greeen, and blue, and fill the chart with every possible
    combination of 0 and 1. Then, try each combination on the LEDs,
    and label each colour produced.

    What RGB values correspond to the RGB LED being off? How many 
    colours can the RGB LED produce using only simple 0 and 1 output
    values for control? (We'll explore how you can display many more
    colours in a later activity!)
    
4.  Create a program that either generates a unique flashing pattern
    on BEAPER Nano's LEDs, or that creates a colour pattern using the
    Arduino Nano ESP32's RGB LED, or that plays one or more tones
    (or any combination of these) for two or more of BEAPER Nano's
    pushbuttons.
    ...
    same time, or does each pattern play n succession? Explain why
    this is the case.
...
8.  Let's imagine that you're ready to apply your newly-acquired
    ...
    designed a 3D-printed holder to mount an Arduino Nano ESP32,
